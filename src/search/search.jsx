...
export function Search() {
  ...
  async function populateGames() {
    try {
      const response = await fetch('/games copy.json');
      ...
      return (
        <main className="maintext">
          <div>
          <h2>Search</h2>
          <p>Sorry, there was an error loading the games. Please try again later.</p>
          </div>
        </main>);
    }
  }
            ...
            <table>
                <tbody>
                    {games.map((game, index) => (
                        <tr key={index}>
                            <td><img className="gameimg" src={game.image} alt={game.name} width="100"></img></td>
                            <td>{game.name}</td>
